...
void findAndReplaceInFile()
{
    ...
    int word_len, i, p = 0;
    float label, pixel1, pixel2, pixel3, pixel4, pixel5, pixel6;
 
    ifp = fopen("arquivo.txt", "r");
    ofp = fopen("file_replace_output2.txt", "w+");
    ...
    rewind(ifp);
    while (!feof(ifp)) { 
        fscanf(ifp, "%f %f %f %f %f %f %f", &label, &pixel1, &pixel2,
        &pixel3, &pixel4, &pixel5, &pixel6); 
